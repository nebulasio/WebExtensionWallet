...
function getTokenDecimal(token) {
...
function onUnlockFile(swf, fileJson, account, password) {
  ...
  try {
    ...
    bootbox.dialog({
      ...
      onEscape: true,
      message:
        localSave.getItem("lang") == "en"
          ? e
          : "keystore 文件错误, 或者密码错误",
      size: "large",
      title: "Error",
    });
  }
...
function updateBalance() {
  ...
  if (currency == "NAS") {
    ...
    neb.api
      .call({
        from: addr,
        to: uiBlock.getContractAddr(uiBlock.currency),
        gasLimit: 400000,
      ...
      .then(function (resp) {
        ...
        var b = resp.result.replace(/"/gi, "");
        // let decimals = uiBlock.currency == "NAX" ? "gwei" : "nas";
        // var balance = Unit.fromBasic(b, decimals).toString(10);

        let decimals = getTokenDecimal(uiBlock.currency);

        let decimalsBig = Utils.toBigNumber(10).pow(decimals);

        let balance = Utils.toBigNumber(b).dividedBy(decimalsBig).toString(10);
...
function onClickGenerate() {
  ...
  if (validateAll()) {
    ...
    gasprice = $("#price").val();
    nonce = $("#nonce").val();
    if ($("#contract").val()) contract = JSON.parse($("#contract").val());
    ...
    if (
      ...
      try {
        var tmp = Unit.fromBasic(
          Utils.toBigNumber(gaslimit).times(Utils.toBigNumber(gasprice)),
          "nas"
        );
        ...
        if (uiBlock.currency == "NAS" || contract) {
          ...
          gTx = new Transaction(
            parseInt(localSave.getItem("chainId")),
            gAccount,
            toAddress,
            Unit.nasToBasic(Utils.toBigNumber(amount)),
            parseInt(nonce),
            gasprice,
            gaslimit,
            contract
          ...
          if (!amountValid)
            throw new Error(
              "Invalid value! The minimum unit is wei (1^-" +
                decimals +
                uiBlock.currency.toLowerCase() +
                ") "
            );
          let value = Utils.toBigNumber(10)
            .pow(decimals)
            .times(amount)
            .toString(10);
          contract = {
          ...
          gTx = new Transaction(
            parseInt(localSave.getItem("chainId")),
            gAccount,
            uiBlock.getContractAddr(uiBlock.currency),
            Unit.nasToBasic(Utils.toBigNumber("0")),
            parseInt(nonce),
            gasprice,
            gaslimit,
            contract
